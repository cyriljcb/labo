...
Voiture::Voiture(string nomV,const Modele & modele)
{
	#ifdef DEBUG
...
void Voiture::RetireOption(string code)
{
	...
	for(i=0;i<5;i++)
	{
		if(i<5&&options[i]!=NULL && (options[i]->getCode() == code))
		{
			delete options[i];
			options[i]=NULL;
			i=10;
...
{
	int taille,cpt = 0;
	string nomfich = nomVoiture + ".car";
	
	ofstream fichier(nomfich,ios::out);
	if(!fichier)
	{
		cout<<"erreur d'ouverture ! "<<endl;
		exit(1);
	}
	taille = nomVoiture.size();
...
void Voiture::Load(string nomfich)
{
	int i,t,taille;
	ifstream fichier(nomfich,ios::in);
	...
	nomVoiture.resize(t);
	fichier.read((char*)nomVoiture.data(),t*sizeof(char));
